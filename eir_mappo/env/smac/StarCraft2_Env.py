...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        self.reward_negative_scale = args.get("reward_negative_scale", 0.5)
        ...
        if self.use_stacked_frames:
            self.stacked_local_obs = np.zeros((self.n_agents, self.stacked_frames, int(
                self.get_obs_size()[0]/self.stacked_frames)), dtype=np.float32)
            self.stacked_global_state = np.zeros((self.n_agents, self.stacked_frames, int(
                self.get_state_size()[0]/self.stacked_frames)), dtype=np.float32)
    ...
    def step(self, actions):
        ...
        try:
            ...
            if self.use_stacked_frames:
                ...
                self.stacked_local_obs[:, -1, :] = np.array(local_obs).copy()
                self.stacked_global_state[:, -1,
                                          :] = np.array(global_state).copy()
        ...
        reward, reward_positive, reward_negative = self.reward_battle()
    ...
    def reward_battle(self):
        ...
        reward = 0
        delta_deaths = 0
        delta_deaths_positive = 0
        delta_deaths_negative = 0
        delta_ally = 0
        delta_enemy = 0
            ...
            if not self.death_tracker_ally[al_id]:
                ...
                if al_unit.health == 0:
                    ...
                    if not self.reward_only_positive:
                        ...
                        delta_deaths_negative -= self.reward_death_value * neg_scale
        ...
        for e_id, e_unit in self.enemies.items():
            if not self.death_tracker_enemy[e_id]:
                ...
                if e_unit.health == 0:
                    ...
                    delta_deaths_positive += self.reward_death_value
        ...
        return reward, abs(delta_enemy + delta_deaths_positive), delta_ally - delta_deaths_negative
    ...
    def get_obs_agent(self, agent_id):
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            if self.obs_pathing_grid:
                move_feats[ind: ind +
                           self.n_obs_pathing] = self.get_surrounding_pathing(unit)
            ...
            for e_id, e_unit in self.enemies.items():
                ...
                if (dist < sight_range and e_unit.health > 0):  # visible and alive
                    ...
                    # available
                    enemy_feats[e_id,
                                0] = avail_actions[self.n_actions_no_attack + e_id]
                    enemy_feats[e_id, 1] = dist / sight_range  # distance
                    enemy_feats[e_id, 2] = (
                        e_x - x) / sight_range  # relative X
                    enemy_feats[e_id, 3] = (
                        e_y - y) / sight_range  # relative Y

                    ind = 4
                    if self.obs_all_health:
                        enemy_feats[e_id, ind] = (
                            e_unit.health / e_unit.health_max)  # health
                        ind += 1
            ...
            # Ally features
            al_ids = [al_id for al_id in range(
                self.n_agents) if al_id != agent_id]
            for i, al_id in enumerate(al_ids):
                ...
                if (dist < sight_range and al_unit.health > 0):  # visible and alive
                    ...
                    if self.obs_all_health:
                        ally_feats[i, ind] = (
                            al_unit.health / al_unit.health_max)  # health
                        ind += 1
        ...
        if self.obs_agent_adversary:
            agent_obs = np.append(agent_obs, [0 for _ in range(self.n_agents)])
    ...
    def get_state(self, agent_id=-1):
        ...
        if self.obs_instead_of_state:
            obs_concat = np.concatenate(
                self.get_obs(), axis=0).astype(np.float32)
            return obs_concat
            ...
            for al_id, al_unit in self.agents.items():
                if al_unit.health > 0:
                    ...
                    if (self.map_type == "MMM" and al_unit.unit_type == self.medivac_id):
                        ally_state[al_id, 1] = al_unit.energy / \
                            max_cd  # energy
                    else:
                        ally_state[al_id, 1] = (
                            al_unit.weapon_cooldown / max_cd)  # cooldown

                    ind = 2

                    if self.add_center_xy:
                        ally_state[al_id, ind] = (
                            al_x - center_x) / self.max_distance_x  # center X
                        # center Y
                        ally_state[al_id, ind +
                                   1] = (al_y - center_y) / self.max_distance_y
                        ind += 2
                    ...
                    if unit.health > 0:
                        ind += self.unit_type_bits
                        if self.add_distance_state:
                            ally_state[al_id, ind] = dist / \
                                sight_range  # distance
                            ind += 1
                        if self.add_xy_state:
                            ally_state[al_id, ind] = (
                                al_x - x) / sight_range  # relative X
                            # relative Y
                            ally_state[al_id, ind +
                                       1] = (al_y - y) / sight_range
                            ind += 2
                        if self.add_visible_state:
                            if dist < sight_range:
                ...
                if e_unit.health > 0:
                    ...
                    if self.add_center_xy:
                        enemy_state[e_id, ind] = (
                            e_x - center_x) / self.max_distance_x  # center X
                        # center Y
                        enemy_state[e_id, ind +
                                    1] = (e_y - center_y) / self.max_distance_y
                        ind += 2
                    ...
                    if unit.health > 0:
                        ...
                        if self.add_enemy_action_state:
                            # available
                            enemy_state[e_id,
                                        ind] = avail_actions[self.n_actions_no_attack + e_id]
        ...
        if self.obs_agent_adversary:
            state = np.append(state, [0 for _ in range(self.n_agents)])
    ...
    def get_state_agent(self, agent_id):
        ...
        if (self.use_mustalive and unit.health > 0) or (not self.use_mustalive):
                ...
                if al_unit.health > 0:  # visible and alive
                    if unit.health > 0:
                        if dist < sight_range:
                            ally_feats[i, 0] = 1  # visible
                        ally_feats[i, 1] = dist / sight_range  # distance
                        ally_feats[i, 2] = (al_x - x) / \
                            sight_range  # relative X
                        ally_feats[i, 3] = (al_y - y) / \
                    ...
                    if self.add_center_xy:
                        ally_feats[i, ind] = (
                            al_x - center_x) / self.max_distance_x  # center X
                        # center Y
                        ally_feats[i, ind+1] = (al_y -
                                                center_y) / self.max_distance_y
                        ind += 2
            ...
            if self.add_center_xy:
                own_feats[ind] = (x - center_x) / \
                    self.max_distance_x  # center X
                own_feats[ind+1] = (y - center_y) / \
                    self.max_distance_y  # center Y
                ind += 2
    ...
    def get_obs_size(self):
        ...
        return [all_feats * self.stacked_frames if self.use_stacked_frames else all_feats, [n_allies, n_ally_feats], [n_enemies, n_enemy_feats], [1, move_feats], [1, own_feats+agent_id_feats+timestep_feats+self.n_agents]]
    ...
    def get_state_size(self):
        ...
        if self.use_global_state:
            ...
            return [size * self.stacked_frames if self.use_stacked_frames else size, [self.n_agents, nf_al], [self.n_enemies, nf_en], [self.n_agents, nf_mv_glb if self.add_move_state else 0], [1, timestep_state], [1, info_state+self.n_agents]]
        ...
        return [size * self.stacked_frames if self.use_stacked_frames else size, [self.n_agents, nf_al], [self.n_enemies, nf_en], [1, move_state + obs_agent_size + timestep_state + agent_id_feats + self.n_agents]]
    ...
    def get_visibility_matrix(self):
        ...
        """
        arr = np.zeros((self.n_agents, self.n_agents +
                       self.n_enemies), dtype=bool)
